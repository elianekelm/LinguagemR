...
barplot(vetor_total_resultados,
        ...
        main = "Barplot em R",
        ylim = c(0,60))
dev.off()  #função que salva em disco
...
dados = data.frame(group = c("A", "B", "C", "D"), value = c(33,62,52,67))
View(dados)
...
fatias <- c(4, 12, 14, 16, 8)
paises <- c("Brail", "Estados Unidos", "Alemanha", "Reino Unido", "Espanha")
...
##### SCATTER PLOT  #####
# --------> serve para análise bivariada (relação entre duas variáveis)
...
# Adiciona smooth (um intervalo de confiança)
ggplot(data, aes(x=my_x, y=my_y)) +
        geom_point(shape=1) +
...
# Customizando (trocando a ordem do Grupo/Subgrupo)
treemap(dados,
        index = c("grupo", "subgrupo"),
        vSize = "valor",
        type = "index",
...
# dataset
# Função rnorm --> cria uma variavel seguindo uma distribuiçao normal com valores
# aleatorios
